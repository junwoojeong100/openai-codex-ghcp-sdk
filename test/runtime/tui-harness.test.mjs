...
for (const scenarioId of ["U01", "U02", "U11", "U12"]) {
  test(`${scenarioId}: the real Codex TUI through bin/codex-ghcp and headless Playwright, with an SDK double`, { timeout: 180_000 }, async t => {
    ...
    const result = await runCase({ directory: path.join(root, "evidence"), workRoot: path.join(root, "work"),
      model: scenarioId === "U02" ? "claude-haiku-4.5" : "gpt-6-astra", scenarioId,
    ...
    assert.deepEqual(result.checks.filter(check => !check.passed).map(check => check.id), ["mcp-isolation"], JSON.stringify({ result,
      http: facts.observer.http.map(({ body, ...row }) => row), diagnostics: facts.observer.diagnostics,
      sdk: facts.observer.sdk.filter(row => row.type !== "models.list"), answers: facts.observer.answers }));
