...
if (process.env.GHCP_TERMINAL_FIXTURE && path.resolve(process.argv[1] || "") === path.join(root, "src/server.mjs")) {
  ...
  const record = { executionKind: "offline-terminal-fixture", sends: 0, sessions: 0, aborts: 0, diagnostics: [] };
  ...
  SessionManager.prototype.start = async function () {
    this.clientFactory = () => new FakeClient({
      ...
      onSend(session, { prompt }) {
        ...
        const answer = /\b(?:FIRST_OK|AFTER_IDLE_OK|AFTER_INTERRUPT_OK|AFTER_SETUP_OK)\b/.exec(text)?.[0];
        if (answer) { session.reply(answer); return; }
        if (!/\b(?:IDLE_FIXTURE|INTERRUPT_FIXTURE)\b/.test(text)) throw new Error("Unexpected terminal fixture request");
