...
The launcher keeps Codex's automatic HTTP/stream retries disabled. The bridge checks SDK readiness and records content-free turn watchdog diagnostics every **15 seconds** (`SDK_READINESS_INTERVAL_MS`). Each attempt initially allows **180 seconds for first model progress** (`TURN_FIRST_PROGRESS_TIMEOUT_MS=180000`), including prompt processing and silent reasoning. `assistant.turn_start` only reports initialization; repeated turn starts, SDK retries and keepalives do not restart this allowance. After real progress, the **90-second** inactivity deadline (`TURN_IDLE_TIMEOUT_MS=90000`) applies. Root text, reasoning, tool-input streaming and increasing `assistant.streaming_delta` byte counts refresh it. Registered root Fusion phases also contribute increasing private-output byte counts and one successful completion receipt per phase; duplicate, review and subordinate activity do not. Private content is never forwarded. A completely silent upstream still cannot be distinguished from long, unobservable reasoning by a local health check.
...
**Repeated 409 after tools finish:** returning every pending result now permits a trusted instruction/model/context/tool update without `/new`. The non-instruction history must still match. After confirmed old-session cleanup and SDK readiness, the bridge carries completed results into a replacement session as history, never as duplicate result RPCs. Missing, duplicate or mismatched results and rewritten history remain rejected. This is not an exactly-once guarantee against a model proposing the same operation with a new call ID. See [handoff boundaries](docs/ARCHITECTURE.md#conversation-continuity).

The focused instruction-handoff regression passed **6/6 real models** through the actual Codex TUI and headless Playwright: one fixture execution per model, zero old result-RPC submissions, and a successful follow-up in the same thread. This is not a rerun of the full stability/TUI matrices. [Evidence, preserved initial failures and limits](docs/validation/2026-09-23-pending-handoff.json).

The subsequent full release check passed **424 unit/integration tests and 72/72 live TUI cases**, but default **v5 stability remains 57/66**, with nine explicit Opus upstream-filter failures. No failure was waived; publication was initially withheld and later explicitly authorized. The same [verification record](docs/validation/2026-09-23-pending-handoff.json) preserves both matrices separately.

After publication, Linux verification defects were repaired and all six Linux/macOS CI jobs passed. A separate, opt-in `application-data-v4` contract clarifies whole-file/label preservation; its fresh full run reached **65/66 (98.48%)**, with one Opus S11 upstream filter still failed. Local unit/integration checks passed **448/448**. This does not regrade the unchanged default v5 contract or promise to eliminate provider refusals. [Changes, preserved failures and evidence](docs/validation/2026-09-23-failure-iterations.json).
...
**Source edits do not update an already-running bridge.** With the default foreground launcher, exit Codex normally and run `./bin/codex-ghcp -- resume --last` from the same directory to load the fix and resume the latest conversation. Do not kill the bridge beneath an active Codex session. `codex-ghcp-status` reports background bridges only; `stopped` does not mean that a foreground bridge is absent. A running bridge's `/health` exposes `turnWatchdog` (`firstProgressTimeoutMs`, `idleTimeoutMs`, `recoveryAttempts`, `intervalMs`), also included in background status. Missing `firstProgressTimeoutMs` means that process has not loaded the split-deadline fix. To opt into the former initial allowance, set `TURN_FIRST_PROGRESS_TIMEOUT_MS=90000`. Recovery skips report a specific reason instead of an ambiguous claim that no request was retried.

**If automatic recovery is exhausted:** the replacement session also made no observable progress before its applicable limit. This does not prove a deadlock: `/health.ready` only checks the local SDK connection, not reachability of the Copilot model service. Connection errors such as `Connect: ... ETIMEDOUT` require checking the network, proxy/VPN and service availability; increasing retries cannot repair an outage. Startup failures identify the SDK operation (`start`, `ping` or `listModels`). Watchdog diagnostics and idle errors distinguish `first_progress` from `streaming`; available root `model.call_failure` metadata records only a bounded API/transport category and HTTP status, never provider text or correlation IDs. Those failures do not masquerade as model progress or interrupt the SDK's own retry. The original five-minute turn budget still covers recovery: a replacement does not automatically get another full three minutes. An absolute deadline error reports how many recoveries were attempted.

For slower workloads, an **opt-in latency-tolerant launch** is available after normal exit. This allows three minutes both before and after first progress, at most two safe recoveries, a ten-minute absolute turn limit and an eleven-minute total request limit:
...
npm run test:ci                       # Unit source coverage + scenario/document consistency
npm run test:runtime                  # All offline Codex/PTY/browser and workflow/stability checks
...
The separate `codex-ghcp-tui-12-v3` contract drives the **real TUI → bridge → Copilot SDK → exact model** path with headless Playwright/xterm.js: **12 scenarios × 6 models = 72 cases**, targeting **95% (69/72)** in one complete live run. It covers picker switching, shell and `apply_patch` tools, Codex MCP isolation, long output, large pastes, Escape recovery, `/compact`, resume, effort changes and cleanup. Common checks also require authoritative SDK model/context-tier snapshots, successful Responses SSE, split first-progress/streaming watchdog settings and clean shutdown. Run `npm run test:tui` (plan), `npm run test:tui:runtime` (offline) or `npm run test:tui -- --execute`. See [real TUI scenarios](docs/TUI_SCENARIOS.md). V2 records remain historical and are verified with their frozen source, not regraded as v3.

**Published historical TUI result (2026-09-23 KST): v2 72/72 (100%)**, all six models 12/12, on implementation `fe500d78`; then-current and frozen-source verification passed. The first run's **45/72** remains recorded, including a Copilot API connection timeout. This predates the split first-progress deadline and is not a v3 result or a guarantee of future network availability. See the [v2 evidence and limitations](docs/validation/2026-09-23-tui-connection-v2/README.md). [Historical v1 70/72](docs/validation/2026-09-23-tui-scenarios/README.md) is a separate contract, not regraded or combined.
...
**Latest split-deadline validation (2026-09-23 KST): v3 72/72 (100%)** in a fresh live TUI matrix, independently verified from current and frozen source. The exact late-first-progress regression also completed after **95 measured seconds** in the actual TUI with an SDK double, one SDK session and no replay. This does not establish the cause of the user's separate upstream stall or guarantee future service availability. [Evidence and remaining limits](docs/validation/2026-09-23-first-progress.json).

GitHub Actions runs unit checks and Node's built-in source coverage on Linux/macOS with Node 22/24, plus separate Linux/macOS jobs using pinned Codex and SDK doubles for actual TUI/PTY/browser and workflow regressions. No Copilot credentials or live model calls are used in CI. `coverage/lcov.info` measures the `src` modules observed by the unit-test runner; it does not include unexecuted entry points or certify product-feature coverage. A workflow definition alone is not proof that those remote jobs have passed. Verification reports separately record the local OS/architecture/Node environment and proxy presence without addresses or secrets.

The separate `codex-ghcp-stability-11-v5` contract has **11 scenarios × 6 models = 66 cases**. It exercises tool reordering, policy rejection with missing results, HTTP duplication/cancellation, SDK loss, stream mismatch, resume and compaction through real Codex. v5 versions the S03 boundary; archived v4 scores are not regraded. Fault injections and actual model results are distinguished. See the [scope, acceptance, settings and commands](docs/STABILITY_TESTING.md).
...
**Historical full real-Codex verification (six-model contracts, 2026-09-23 KST): default v4 57/66 (86.36%); separate `application-data-v2` 63/66 (95.45%).** Both entire matrices ran on implementation `68f92d74` and were verified against current/frozen source; they predate the freeform `apply_patch` catalog change and were not rerun on `54c7eb77`. In v4, every model except `claude-opus-5.5` passed 11/11; all nine Opus 5.5 failures are explicit upstream filters. The remaining application-data-v2 failures are one Opus filter, one SDK `disconnect` cleanup timeout and one model tool-repetition miss. The bridge now disables the Copilot runtime's unused MCP servers (0 MCP processes across the final matrices). Live runs also found and fixed a macOS `EPERM` supervisor false failure. See the [changes, run history and evidence](docs/validation/2026-09-23-six-model-switch/README.md).
