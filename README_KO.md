...
이후 배포 전 전체 확인에서 **단위·통합 424건과 실모델 TUI 72/72**가 통과했지만 기본 **v5 안정성은 57/66**이며 Opus의 명시적 상위 필터 실패 9건이 남았습니다. 실패를 통과로 처리하지 않고 당시 조건부 커밋·푸시를 보류했으며, 이후 명시적 승인을 받아 게시했습니다. 두 행렬은 같은 [검증 기록](docs/validation/2026-09-23-pending-handoff.json)에 별도로 보존합니다.

게시 후 Linux 검증기 결함을 수정하여 Linux/macOS CI 작업 6개가 모두 통과했습니다. 파일 전체·레이블 보존을 명확히 한 별도 선택형 `application-data-v4`의 새 전체 실검증은 **65/66(98.48%)**이며 Opus S11 상위 필터 1건은 실패로 유지합니다. 로컬 단위·통합도 **448/448** 통과했습니다. 기본 v5 계약을 재채점하거나 공급자 거절의 완전한 해결을 뜻하는 결과는 아닙니다. [변경·보존한 실패·증거](docs/validation/2026-09-23-failure-iterations.json).
