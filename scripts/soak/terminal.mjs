...
export class TerminalScreen {
  ...
  #linefeed() {
    if (++this.row >= this.rows) { this.lines.shift(); this.lines.push(Array(this.columns).fill(" ")); this.row = this.rows - 1; }
  }
  #eraseLine(mode) {
  ...
  #csi(parameters, command) {
    ...
    else if (command === "L") for (let i = 0; i < Math.min(first, this.rows - this.row); i++) { this.lines.splice(this.row, 0, Array(this.columns).fill(" ")); this.lines.pop(); }
    else if (command === "M") for (let i = 0; i < Math.min(first, this.rows - this.row); i++) { this.lines.splice(this.row, 1); this.lines.push(Array(this.columns).fill(" ")); }
    else if (command === "S") for (let i = 0; i < Math.min(first, this.rows); i++) { this.lines.shift(); this.lines.push(Array(this.columns).fill(" ")); }
    else if (command === "T") for (let i = 0; i < Math.min(first, this.rows); i++) { this.lines.pop(); this.lines.unshift(Array(this.columns).fill(" ")); }
...
export function inspectTerminalScreen(text, { knownCodex = false, prompt = "", expectedMarker = "" } = {}) {
  ...
  return { ready: Boolean(codex && composer && !busy && !trust && !theme && !permission && !login),
    markerObserved: Boolean(marker), trust, theme, permission, login, busy };
}
...
export async function runTerminalProbe({
  ...
  const requests = (turns ?? [{ prompt, expectedMarker }]).map((turn, index) => {
    if (typeof turn.prompt !== "string" || !turn.prompt.trim() || /[\x00-\x08\x0b-\x1f\x7f]/.test(turn.prompt) || Buffer.byteLength(turn.prompt) > 16_384) throw new Error(`Invalid prompt for turn ${index}.`);
    if (!/^[A-Za-z0-9_.-]{1,128}$/.test(turn.expectedMarker ?? "") || turn.expectedMarker.length > columns - 4) throw new Error(`Invalid expectedMarker for turn ${index}.`);
  ...
  if (!requests.length || new Set(requests.map(turn => turn.expectedMarker)).size !== requests.length) throw new Error("Turns require distinct completion markers.");
  ...
  const startedAt = Date.now(), started = performance.now(), screen = new TerminalScreen({ rows, columns }), decoder = new StringDecoder("utf8");
  const result = { status: "failed", reason: null, startedAt: new Date(startedAt).toISOString(), turns: [],
    terminal: { rows, columns, privatePty: true }, artifacts: { raw: rawFile, events: eventFile },
  ...
  let stopped = false, bytes = 0, lineBuffer = "", active = null, index = 0, lastOutput = started;
  let nextAt = started + requests[0].delayMs, readySeen = false, trustHandled = false, setupHandled = false, fallbackTimer;
  const send = message => {
  ...
  helper.stdout.on("data", data => {
    ...
    while ((newline = lineBuffer.indexOf("\n")) >= 0) {
      ...
      if (event.type === "data") {
        const chunk = Buffer.from(event.base64, "base64"); bytes += chunk.length; lastOutput = performance.now();
  ...
  const tick = setInterval(() => {
    if (stopped) return;
    const now = performance.now(), text = screen.text(), request = active ?? requests[index];
    ...
    if (active) {
      if (state.markerObserved && state.ready && now - lastOutput >= 150) {
        const turn = result.turns.at(-1);
        Object.assign(turn, { completedAt: new Date().toISOString(), durationMs: Math.ceil(now - active.sentAt), markerObserved: true });
        record({ type: "turn-completed", index, expectedMarker: active.expectedMarker, screenHash: hash(text) });
        index++; active = null;
        if (index === requests.length) { stop("all-markers-observed", "passed"); return; }
        nextAt = now + requests[index].delayMs;
      } else if (now - active.sentAt >= active.timeoutMs) stop("completion-deadline", "timed-out");
    } else if (state.ready && now >= nextAt && now - lastOutput >= 150) {
      const turn = requests[index];
      if (inspectTerminalScreen(text, { expectedMarker: turn.expectedMarker }).markerObserved) { stop("stale-completion-marker"); return; }
      if (turn.prompt.includes("\n") && !screen.bracketedPaste) { stop("multiline-paste-not-supported"); return; }
      active = { ...turn, sentAt: now };
      result.turns.push({ index, promptHash: hash(turn.prompt), expectedMarker: turn.expectedMarker,
