...
if (process.env.GHCP_SOAK_OBSERVER && path.resolve(process.argv[1] || "") === path.join(ROOT, "src/server.mjs")) {
  ...
  const records = { push(row) {
    ...
    } else if (row.type === "assistant.message" && root && roles.get(row.sessionId) === "compaction") {
      if (row.data.content && !row.data.toolRequests?.length) append("summaries.jsonl", { sessionId: row.sessionId, content: row.data.content });
    } else if (row.type === "session.error" && root) {
      metrics.errors++; append("sdk.jsonl", { type: row.type, sessionId: row.sessionId,
