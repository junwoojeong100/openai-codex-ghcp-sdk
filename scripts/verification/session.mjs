...
import { inspectTerminalScreen } from "./terminal.mjs";
export { descendants, processTable } from "./processes.mjs";

const hash = value => createHash("sha256").update(value).digest("hex");
const sleep = ms => new Promise(resolve => setTimeout(resolve, ms));
const PASSTHROUGH = ["COPILOT_GITHUB_TOKEN", "GH_TOKEN", "GITHUB_TOKEN", "GH_ENTERPRISE_TOKEN", "GITHUB_ENTERPRISE_TOKEN", "GH_CONFIG_DIR", "HTTPS_PROXY", "HTTP_PROXY", "ALL_PROXY", "https_proxy", "http_proxy", "all_proxy", "COPILOT_CLI_PATH"];
...
export function readRollouts(codexHome) {
  ...
  const commandText = argv => Array.isArray(argv) ? argv.length === 3 && /^-(?:l?c|cl)$/.test(argv[1])
    ? `${argv[0]} ${argv[1]} ${JSON.stringify(argv[2])}` : argv.join(" ") : argv;
  for (const file of files.sort()) for (const line of fs.readFileSync(file, "utf8").split("\n")) {
    ...
    if (row.type === "event_msg" && payload.type === "exec_command_end") {
      ...
      summary.commands.push({ callId: payload.call_id, command, exitCode: payload.exit_code,
    ...
    if (row.type === "event_msg" && payload.type === "item_completed" && typeof payload.item?.type === "string") {
      ...
      if (/^command_?execution$/i.test(payload.item.type)) {
        const item = payload.item;
        summary.commands.push({ callId: item.id, command: commandText(item.command), exitCode: item.exitCode ?? item.exit_code,
...
export class TuiSession {
  constructor({ directory, model, ownedRoot, env = process.env, rows = 45, columns = 140, executionKind = "live", preload, codexArgs = [], sandbox = "read-only" }) {
    ...
    this.steps = []; this.launches = []; this.samples = []; this.sampleErrors = []; this.closed = false;
  ...
  childEnv() {
    ...
    Object.assign(env, { CODEX_BIN: this.env.CODEX_BIN || "codex", TERM: "xterm-256color", COLUMNS: String(this.columns), LINES: String(this.rows),
      ...
      NODE_OPTIONS: `--import=${JSON.stringify(fileURLToPath(new URL("./observer.mjs", import.meta.url)))}${this.preload ? ` --import=${JSON.stringify(this.preload)}` : ""}` });
  ...
  async ask(prompt, marker, timeoutMs = 240000) {
    ...
    return this.waitFor(`answer-${marker}`, text => {
      ...
      if (requests.some(row => row.invalidCompletion)) throw new Error("Invalid completed-response evidence");
  ...
  async closeLaunch() {
    ...
    try {
      if (this.renderer) this.launches.at(-1).media = await this.renderer.close();
    } catch (error) { this.launches.at(-1).rendererCloseError = error.message; }
    this.renderer = null;
  }
  leftovers() {
  ...
  observer() {
    ...
    return { sdk: read("sdk.jsonl"), answers: read("answers.jsonl"), summaries: read("summaries.jsonl"), http: read("http.jsonl"), diagnostics: read("diagnostics.jsonl"),
