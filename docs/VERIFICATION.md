...
The repair prompt explicitly asks the model to re-read `sample.txt` after patching and before the final test command. Its hidden value is independent of the visible baseline acknowledgment, so changing that acknowledgment's counter cannot guess it. `V02.edit` measures the source change; `V02.answer` separately requires the exact file value. A correct patch with a wrong answer still fails the case, without a repair prompt or answer substitution.
...
V02 checks actual failing/passing test summaries and native exit codes, not shell formatting. Read-only preparation (including `echo`/`printf` labels) may precede the test command when it remains the final command; pipes, exit masking and fake test output are not accepted. A completed tool-call response is a handoff, not a final answer. The runner requires a completed text response, fresh matching native `task_started`/`task_complete` receipts and a ready composer before continuing; a rendered answer alone does not make `/compact` available. A rejected compact command fails without retrying it.

Answer checks compare the entire final client-visible text (apart from surrounding whitespace) and the last fresh SDK answer, not a marker in earlier commentary or an otherwise incorrect response. Correct text may wait for delayed terminal rendering; a renderer failure fails immediately, and capture errors identify the checkpoint. Failed answers remain in the case facts. HTTP/SDK observers record correlated tool-result hashes rather than another copy of the text, allowing comparison with native results to distinguish faithful delivery from an incorrect model reply. An unrecovered startup-catalog error also fails the upstream check; catalog recovery needs ordered receipts for the failed and replacement generations in the same bridge process.

Native command-completion events may omit output already streamed, even when the actual tool result contains it. File-read evidence therefore uses the native `function_call_output`/`custom_tool_call_output` associated with the call ID; native command events still establish the command and exit status. The driver does not manufacture missing output or treat a partial command summary as the full tool result.
...
Verification launches pass `-c check_for_update_on_startup=false` because the CLI version is pinned. This prevents a cached update notice from replacing the composer after a cold resume. Numbered menu choices are not accepted as a composer, and an unexpected update popup fails without selecting an option. The setting is confined to verification and offline fixtures; normal launcher behavior and user/global installations are unchanged.
