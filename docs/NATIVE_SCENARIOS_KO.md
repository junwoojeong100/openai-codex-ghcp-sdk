...
**bridge 실패 위험:** provider 혼동·fallback·SSE 손실

<a id="c02"></a>

### C02 — AGENTS 지시문·Skill 실행·비신뢰 입력 방어

**제한 시간 / 실행 경로:** 120s · codex-app-server
...
**bridge 실패 위험:** 지시 계층 손실·Skill 문맥 누락

<a id="c03"></a>

### C03 — 저장소 탐색·코드 이해·Git diff 리뷰

**제한 시간 / 실행 경로:** 90s · codex-app-server
...
**bridge 실패 위험:** 잘못된 파일·줄·이력 해석과 리뷰 환각

<a id="c04"></a>

### C04 — 다중 파일 리팩터링·생성·이동·삭제·Git 보호

**제한 시간 / 실행 경로:** 150s · codex-app-server
...
**bridge 실패 위험:** patch/경로 손상·사용자 변경 덮어쓰기
...
**bridge 실패 위험:** 도구 오류·exit 유실·실패 로그의 잘못된 재사용
...
**bridge 실패 위험:** additional_tools/schema/namespace 변환·오류 결과 손실
...
**bridge 실패 위험:** SDK 권한과 Codex 승인 혼동·선실행

<a id="c08"></a>

### C08 — 파일·네트워크 샌드박스와 프로세스 정리

**제한 시간 / 실행 경로:** 60s · codex-app-server
...
**bridge 실패 위험:** 도구 실행 소유권 이동으로 보안 경계 무력화
...
**bridge 실패 위험:** prefix 재생 오류·SDK 세션 공유·지시 업데이트 손실
...
**bridge 실패 위험:** in-memory response ID 의존·이력 역할 손실

<a id="c11"></a>

### C11 — 생산 launcher 기본 도구·프로세스 수명

**제한 시간 / 실행 경로:** 120s · production-launcher
...
**bridge 실패 위험:** 네이티브 동작을 우회하거나 대역/부분 성공을 실모델 통과로 오인
...
**제한 시간 / 실행 경로:** 180s · codex-app-server
...
- 동일한 작은 합성 fixture와 지시문을 6모델에 사용한다. 모델별 도구/파일 결과를 독립 oracle로 판정하며 숨은 nonce는 프롬프트에 넣지 않는다.
