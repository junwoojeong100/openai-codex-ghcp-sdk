...
보고서의 `executionKind`로 오프라인과 실모델을 구분하세요. **이 워크플로 실행기는 소스 지문만 기록하며 `source-snapshot` 디렉터리를 만들지 않습니다.** 소스를 수정하기 전에 검증하세요. 나중에도 검증하려면 원래 실행 디렉터리 전체와 당시의 정확한 소스·의존성 잠금 파일 사본을 별도로 보관해야 합니다.
