...
`codex-ghcp-workflows-18-v5`(schemaVersion 5): **시나리오 18개 × GHCP 모델 6개 = 108건**입니다. 행렬 크기·리포트 분모·생성 문서는 카탈로그에서 계산합니다. 실검증 부분 선택·자동 케이스 재실행·OpenAI 기준선 실행은 없습니다.

과거 v4 126건 보고서는 원래 runner revision으로만 검증합니다. 기존 검증 결과와 원본 증거는 사용자 요청으로 삭제했습니다. [새 검증 기록](validation/README_KO.md)을 참고하세요. 현재 재실행 대상은 별도 안정성 11개 시나리오이며, 그 결과를 이 18개 워크플로 계약의 통과 증거로 사용할 수 없습니다.
...
## 실행 전에 고정한 증거 해석(v4에서 v5로 변경 없음)
...
- 모두 v4에서 고정해 v5에 그대로 적용하는 규칙이며 실행 후 추가한 예외가 아닙니다. 실검증 전에 카탈로그·구현 해시를 고정하고 실패를 보존하며, 점수를 올리기 위한 개별 재실행은 하지 않습니다.
