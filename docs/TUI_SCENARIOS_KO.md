...
Codex의 대화형 터미널 화면(TUI)에서 모델 전환·도구·중단·압축·재개를 검사합니다. 실모델 검사는 **실제 Codex TUI → 운영 bridge → Copilot SDK → 지정 모델** 경로를 사용합니다.

시나리오와 통과 기준의 버전인 **검사 사양(contract)**은 `codex-ghcp-tui-12-v3`이며 **12개 시나리오 × 6개 모델 = 72건**입니다. 한 건은 한 모델에서 시나리오 하나를 실행한 결과입니다. 변경되지 않은 구현의 완전한 실모델 실행 한 회에서 **69/72**는 95% 목표 달성, **72/72**는 전체 통과입니다. 안정성·호환성·이전 TUI 실행의 결과와 합산하지 않습니다.

**바로가기:** [실행](#준비와-실행) · [결과 읽기](#결과-읽기) · [시나리오 목록](#시나리오) · [통과 기준](#판정) · [검증과 종료 코드](#과거-실행-검증과-종료-코드).

## 준비와 실행

`npm ci` 후 저장소 루트에서 실행합니다. 목적에 맞는 모드를 고르세요. 각 모드는 단독으로 실행할 수 있습니다.

| 목적 | 실행 모드 | 실모델 호출 |
| --- | --- | --- |
| 시나리오 확인 | [계획](#계획) | 없음 |
| 모의 SDK 응답으로 TUI 검사 러너 확인 | [오프라인 runtime](#오프라인-runtime) | 없음 |
| Copilot 모델로 72건 모두 측정 | [실모델 행렬](#실모델-행렬) | **있음** |
| 원본 증거로 기존 보고서 재계산 | [저장된 보고서 검증](#저장된-보고서-검증) | 없음 |

### 계획

**모델 호출·Codex 설치·브라우저·Copilot 로그인이 필요하지 않습니다.** 기본 모드는 행렬 사양만 출력하며 실행하지 않습니다.
...
오프라인·실모델 검사에는 **Node 22.12 이상, Codex 0.154.0, Python 3, headless Chromium**이 필요합니다. [CLI 설치 단계](../README_KO.md#2-의존성-설치)를 따르세요. Copilot 로그인은 실모델 실행에만 필요합니다. 브라우저는 한 번 설치합니다.
...
**모델 호출은 없습니다.** [실행 준비 사항](#실행-준비-사항)을 갖춘 뒤 실제 Codex와 SDK 대역(모델 대신 동작하는 로컬 구현)으로 U01·U02·U11·U12를 검사하세요.
...
**Copilot 사용량이 발생합니다.** [실행 준비 사항](#실행-준비-사항)과 [Copilot 계정 확인](../README_KO.md#3-설치와-계정-접근-확인)을 마치세요. 전체 검사에는 **[지원 모델 6개](../README_KO.md#모델) 모두**가 필요하므로 `./bin/ghcp-models`에서 기본 모델 외의 모델도 사용 가능한지 확인하세요.

새 출력 디렉터리에서 72건 전체를 한 번 실행합니다.
...
**모델 호출이나 케이스 재실행은 없습니다.** 실행할 때와 같은 소스·의존성을 사용하세요. 그 뒤 소스가 바뀌었다면 해당 실행의 [저장된 소스 사본](#과거-실행-검증과-종료-코드)을 사용하세요.

실패한 케이스가 있어도 끝난 실행은 모두 검증하세요. 검증 명령은 따로 실행합니다. 95% 목표에 못 미친 실행은 0이 아닌 코드로 끝나므로 `--execute` 뒤에 `&&`로 연결하면 검증이 건너뛰어집니다. 실행에 사용한 출력 디렉터리를 지정하세요.
...
**종료 코드 0도 72/72를 뜻하지는 않습니다.** 전체 통과는 `fullMatrixPassed`, 95% 목표 달성은 `thresholdMet`로 확인하세요. `evidenceIntegrity`는 검사의 통과가 아니라 증거의 유효성을 뜻합니다. [필드 설명](validation/README_KO.md#결과-읽기)을 참고하세요.

소스가 바뀌었다면 원래 실행 때 저장한 소스로 검증합니다.
...
`cases/`, `freeze.json`, `source-snapshot/`을 포함한 원래 실행 디렉터리 전체와 같은 의존성이 필요합니다. 공개 요약은 원본 증거를 대신하지 못하며, 검증 시 모델 호출은 없습니다.
...
케이스마다 격리된 `HOME`·`CODEX_HOME`·작업 폴더를 쓰며, 실제로 공유하는 것은 Copilot 인증뿐입니다. 승인 정책은 `never`이고 샌드박스는 시나리오마다 고정합니다. 표식은 케이스마다 무작위로 만듭니다. 케이스 자동 재시도, 출력 재작성, 케이스 대체는 없습니다. 운영 bridge의 제한적인 출력 전 복구는 활성화하고 관측하며, 하네스 재시도와 구분합니다. 사용 중인 bridge·사용자 설정은 재시작하거나 변경하지 않습니다.
...
v3는 첫 진행·스트리밍 제한의 분리를 요구합니다. 이전 결과는 당시 동결 소스로만 검증합니다.
...
- **watchdog:** 새 bridge의 `/health`가 첫 진행 180초·스트리밍 무진행 90초·복구 1회·감시 15초를 보고함
- **upstream:** 필터, SDK 오류, 실패 스트림이 없음
- **mcp-isolation:** 표본을 채취하는 동안 bridge Copilot 런타임 아래에 MCP 프로세스가 한 번도 없음
...
기대와 다른 답변으로 완료되거나 HTTP·스트림 오류가 발생하면 원래 증거를 남기고 즉시 실패 처리합니다. 시나리오 제한 시간까지 기다렸다가 무응답으로 오분류하지 않습니다. 실패 전 관측값도 보존합니다. 개선 후에는 새 출력 폴더에서 72건 전체를 다시 실행하며, 다른 구현의 성공 케이스를 합치거나 이전 보고서를 덮어쓰지 않습니다.
