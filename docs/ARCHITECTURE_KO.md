...
| [sdk-lifecycle.mjs](../src/sdk-lifecycle.mjs) | SDK 준비 상태·연결/목록 복구. 무응답·전송 오류 턴 복구와는 별개. |
...
[검증 진입점](../scripts/verify.mjs)과 [핵심 검사 사양](VERIFICATION_KO.md)은 각각 하나입니다. 실제 Codex → 운영 bridge → 실제 Copilot SDK → 정확한 모델 경로를 검사합니다. [구현](../scripts/verification/)은 모델 6개에서 핵심 시나리오 6개를 실행하며, 소스 사본 저장과 fact 재계산까지 자동 수행합니다. 공통 PTY·브라우저 코드는 오프라인 안전성 회귀 검사에서도 사용하지만 별도 실모델 검증은 아닙니다.
...
- 가장 최근에 성공한 정규화 요청의 동일한 재시도는 세션 상태가 살아 있고 유효할 때만 재제출 없이 캐시를 반환합니다. 실패한 요청이나 제거된 세션에는 성공 캐시가 없습니다.
...
- SDK의 send API는 임의의 Responses 이력을 그대로 복원하는 API가 아니므로, 과거 대화가 포함된 최초 요청은 지시문을 제외한 이력을 assistant phase와 함께 문맥으로 직렬화합니다. 마지막 항목이 사용자 메시지이면 그 원문은 과거 JSON에 넣지 않고 이력 뒤에 **현재 요청**으로 한 번만 전달합니다. 압축·모델 변경·안전한 턴 복구에도 같은 경계를 적용합니다.
...
- **하나의 선택 함수:** 같은 선택 함수가 모델 목록 예산과 SDK 세션 설정을 모두 결정합니다. tier는 세션 signature에 포함되므로 추론 수준 변경·이력 재구성·턴 복구 후에도 유지됩니다. 대기 호출이 있을 때 tier를 바꾸려면 위의 완료 결과 핸드오프가 필요하며, 상위 서비스가 거절해도 조용히 다른 tier로 대체하지 않습니다.
- **예산:** 알려진 유효한 prompt·context 한도 중 최솟값을 사용하며, 유효한 문맥·최대 출력 크기가 있으면 출력 공간을 예약합니다. 실행기는 예산이 없거나 잘못되면 거절합니다. Codex는 이 예산의 80%에서 로컬 자동 압축을 시작하며 SDK 자체 압축은 꺼져 있습니다.
...
- **상한:** 세션 수·유휴 시간·본문 크기·이력 크기·턴 시간에 모두 상한이 있습니다. `MAX_TOOL_RESULTS`는 한 턴에서 반환하는 function/custom 호출 전체에도 적용합니다. 초과한 묶음은 호출 게시·캐시 전에 실패하므로, bridge가 결과를 거절할 호출 묶음을 Codex가 실행하지 않습니다.
...
- **종료:** 소유한 모든 SDK client의 종료를 시도하며 정상 정리가 실패하면 제한 시간 안에서 강제 종료합니다. `bridge.upstream_cleanup_failed`에는 작업과 제한된 실패 정보만 기록합니다. 강제 종료도 실패하면 `upstream_cleanup_failed`를 전달하고 서버는 `bridge.shutdown_failed`를 기록한 뒤 종료 코드 1을 반환합니다. 전경 실행기도 Codex의 정상 종료와 무관하게 이 실패를 유지합니다. 정상 종료에 실패해도 강제 종료를 확인하면 종료 코드 0을 허용합니다. 상주 bridge 중지 명령은 프로세스 종료만 확인하므로 SDK 정리 결과는 비공개 bridge 로그의 종료 진단에서 확인합니다.
...
입력 접수가 확인된 무응답 요청은 해당 SDK 세션만 다시 만들어 원래 응답 스트림에서 복구할 수 있습니다. 같은 복구 횟수 한도는 구조화된 루트 `model.call_failure` 전송 오류 뒤에 루트 `session.error`의 `query` 오류로 끝난 경우에도 적용됩니다. API 오류 코드·상태나 모델 진행이 없어야 하며, query 메시지나 연결 오류 문자열만으로 재시도하지 않습니다. 개별 모델 호출 실패는 SDK 자체 복구가 끝날 때까지 진단으로만 취급합니다.

- **전제 조건:** 이전 세션의 abort·disconnect·delete 성공과 같은 client generation의 정상 준비 상태를 먼저 확인합니다. `TURN_IDLE_RECOVERY_ATTEMPTS`는 무응답·전송 오류 복구가 공유하는 한도로 기본 1, 최대 3이며 0이면 이 턴 재시도를 끕니다. 연결·목록 복구나 SDK·공급자 내부 재시도는 제어하지 않습니다.
- **유지하는 것:** 모델·추론 수준·지시문 권한·완료된 전체 이력·완료 호출 식별자·응답 참조 버전·보고된 사용량. 완료된 도구 결과는 문맥으로만 전달하며 **결과 RPC를 다시 제출하지 않습니다.**
- **복구하지 않는 경우:** 부분 출력, 접수 미확인 입력, 대기 호출, 필터링, 취소, 정리 실패, 연결 유실. 전송 오류 복구는 루트 추론·도구 입력·스트리밍 바이트, 사용량 또는 Fusion 출력이 관측된 경우와 API·인증·사용량 제한·할당량 오류, 전달 여부가 불확실한 경우도 제외합니다.
- **제한 시간:** 모든 복구 시도와 대체 세션 설정은 원래 턴·요청 제한을 공유하며 이를 초기화하지 않습니다. 취소는 설정 중인 MCP 확인도 중단합니다.
- **진단:** `bridge.turn_transport_failed`, `bridge.turn_recovering`, `bridge.turn_recovered`, `bridge.turn_recovery_skipped`는 대화 원문 없이 제한된 정보를 남깁니다. 복구 기록에는 정리한 bridge 소유 세션, 원인, 원래 응답 ID가 포함됩니다.
...
- **시작 시 모델 목록 복구:** 첫 `listModels`는 `SDK_STARTUP_TIMEOUT_MS`의 절반(기본 15초)과 남은 시간 안에서만 기다립니다. 제한 시간 만료·식별된 전송 코드·HTTP 500/502/503/504·JSON-RPC 내부/연결 오류이면 `forceStop` 성공 후 새 client를 한 번 사용할 수 있습니다. Start·ping·두 목록 조회와 재시도는 원래 시작 예산(기본 30초)을 공유하며 정리는 별도의 기존 제한을 유지합니다. 식별된 인증·인가·쿼터·요청 한도 거절, 잘못된 인자, 취소, 알 수 없는 오류, 정리 실패, client 재사용은 제외합니다. 이러한 거절 신호는 일반적인 서버·전송 오류 포장보다 우선합니다. 제한된 `catalogFailure` 메타데이터로 판단 근거를 남기되 상위 메시지 원문은 기록하지 않습니다. 늦은 완료는 해당 이전 client만 정리합니다. 이 재시도는 읽기 전용 목록에만 적용하며 추론·도구 결과를 재전송하지 않습니다.
...
- **연결 복구:** `sdk-lifecycle.mjs`는 제한된 ping 검사·단일 연결 복구 작업·새 client generation을 담당합니다. 동시 요청마다 client를 따로 만들지 않고, 시작 제한과 backoff가 있는 공유 복구 작업 하나를 사용합니다. 연결을 잃으면 해당 대화를 무효화하고 명시적으로 실패시키며, 추론이나 불확실한 도구 결과를 다시 보내지 않습니다. 정상 SDK 연결에서 수행하는 위의 제한적 턴 복구와는 별개입니다.
...
- **확정 순서:** 스트림의 delta/final 일치 검사는 성공 캐시나 대기 도구 상태를 확정하기 전에 수행합니다. 성공 결과는 상태가 유효할 때 동일한 재시도에 사용할 수 있으며, 스트림 프로토콜이 맞지 않으면 전달되지 않은 도구 호출을 성공으로 캐시하지 않습니다. 네트워크 전달과 모델 생성은 하나의 정확히 한 번(exactly-once) 트랜잭션이 아닙니다.
...
제한 시간 기본값과 오류 코드는 [사용 안내](USAGE_KO.md#제한-시간과-복구), 현재 연동 검사는 [단일 검증 안내](VERIFICATION_KO.md)에 있습니다. 보존한 최종 실행에는 해당 소스 사본이 포함됩니다.
