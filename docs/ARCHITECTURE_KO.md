...
- **시작 시 모델 목록 복구:** 첫 `listModels`는 `SDK_STARTUP_TIMEOUT_MS`의 절반(기본 15초)과 남은 시간 안에서만 기다립니다. 이 조회의 시간 초과와 `forceStop` 성공이 확인된 경우만 새 client를 한 번 사용하며, start·ping·목록 조회와 재시도는 원래 시작 예산(기본 30초)을 공유합니다. 정리는 별도의 기존 제한을 유지합니다. RPC·인증 오류, 취소, 정리 실패, client 재사용은 복구 대상이 아닙니다. 늦은 완료는 해당 이전 client만 정리하며, 이 복구는 모델 프롬프트를 제출하지 않습니다.
