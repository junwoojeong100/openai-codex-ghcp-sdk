...
승인·샌드박스를 포함한 다른 Codex 옵션은 평소처럼 동작합니다.
...
| `state` | 의미·조치 |
| --- | --- |
| `running` | 등록된 bridge가 응답하고 인증된 모델 목록 조회에 성공했습니다. 모델의 실제 응답을 검사한 것은 아닙니다. |
| `stopped` | 등록된 상주 bridge가 없습니다. foreground bridge는 실행 중일 수 있습니다. |
| `stale` | 등록된 프로세스가 종료됐습니다. 다음 상주 실행이 등록 정보를 교체합니다. |
| `unverified` | 프로세스는 있지만 신원이나 모델 목록 접근을 확인하지 못했습니다. 출력된 `log` 파일을 확인하고, 등록된 PID만 보고 프로세스를 종료하지 마세요. |
...
- bridge는 모델 전환·복구 후에도 항상 해당 모델에 대해 **SDK가 제공하는 가장 큰 문맥 tier**를 사용합니다. 문맥 메타데이터 누락이나 tier 거절은 오류이며, 조용히 다른 값으로 대체하지 않습니다.
- Codex에는 모델의 출력 공간을 남긴 **입력 예산**을 전달하므로 모델의 전체 문맥 창보다 작습니다.
...
실행기는 `~/.codex/config.toml`, `auth.json`, 셸 시작 파일이나 다른 프로젝트의 서버를 수정하지 **않으며**, Copilot·GitHub 로그인을 Codex로 복사하지 않습니다. 다른 Codex 설정은 그대로 적용되며 새 Codex 프로필을 만드는 것도 아닙니다. 파일을 바꾸는 것은 사용자가 직접 하는 두 가지뿐입니다. `/model`은 선택을 `~/.codex/config.toml`에 저장할 수 있고, 선택적인 [zsh 연동](#선택적-zsh-연동)은 `~/.zshrc`를 수정합니다.
...
값은 **밀리초** 단위입니다. 제한들은 동시에 적용되며 서로 더해지지 않습니다. 새 값은 [bridge를 재시작](#안전한-bridge-재시작)해야 적용됩니다.
...
| `TURN_IDLE_RECOVERY_ATTEMPTS` | 1 | 요청당 자동 세션 복구 횟수. 0–3 정수이며 0은 복구를 끕니다. |
...
| `SDK_READINESS_INTERVAL_MS` | 15000 | 백그라운드 연결 검사·턴 감시 진단 간격. 턴 감시 간격은 무응답 제한 이하로 제한됩니다. |
...
`TURN_IDLE_RECOVERY_ATTEMPTS`를 제외한 모든 값은 양의 정수여야 합니다. 나머지 한도와 기본값은 [`.env.example`](../.env.example)에 있습니다.

**자동 복구.** 모델 턴이 제한 시간을 넘겨 응답이 없으면, bridge는 Copilot 세션을 교체하고 같은 응답 스트림에서 턴을 다시 시도할 수 있습니다. 기본값은 **요청당 한 번**입니다.

- **조건:** 잃거나 반복될 작업이 없을 때만 복구합니다. SDK 연결이 정상이고, 입력 접수가 확인됐고, 모델 출력이나 도구 호출이 아직 없고, 이전 세션 정리가 확인돼야 합니다.
- **그대로인 것:** 원래 턴·요청 제한 시간. 완료된 도구 결과는 다시 보내지 않습니다.
- **비용:** 재시도에 추가 Copilot 사용량이 발생할 수 있습니다.
- **끄기:** `TURN_IDLE_RECOVERY_ATTEMPTS=0`을 지정하세요. Codex 자체의 HTTP·스트림 자동 재시도는 항상 꺼져 있습니다.
...
제한 시간 초과 시 조치는 [느린 응답과 연결 장애](#느린-응답과-연결-장애)를 참고하세요.
...
| `copilot_idle_timeout`, `copilot_timeout`, `request_timeout`, `ETIMEDOUT` | 먼저 네트워크·프록시·서비스 상태를 확인한 뒤 [느린 응답과 연결 장애](#느린-응답과-연결-장애)와 [제한 시간](#제한-시간과-복구)을 참고하세요. 로컬 bridge가 정상이어도 모델 서비스를 사용할 수 있다는 뜻은 아닙니다. |
...
검사 명령과 기록된 결과는 [검증 안내](../README_KO.md#개발과-검증)와 [검증 기록](validation/README_KO.md)을 참고하세요. 기록은 실패를 보존하고 오프라인 검사·실모델 행렬·과거 구현을 구분합니다.
