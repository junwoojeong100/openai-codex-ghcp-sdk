...
| `MAX_TOOL_RESULTS` | 32 | 한 턴에서 반환하는 function/custom 호출 전체와 다음 요청에 함께 접수할 결과 수. 초과한 호출 묶음은 전달 전에 실패합니다. |
...
| `tool_call_limit_exceeded` | 502 | 모델이 `MAX_TOOL_RESULTS`보다 많은 호출을 반환했습니다. 호출은 전달하지 않았으며, 결과를 나누지 말고 더 작은 작업 묶음을 요청하세요. |
...
시작 중 `listModels`의 시간 초과 또는 식별된 전송·서버·JSON-RPC 내부 오류는 정리 성공을 확인한 뒤 같은 `SDK_STARTUP_TIMEOUT_MS` 예산(기본 30초) 안에서 SDK client를 한 번 교체할 수 있습니다. 첫 목록 조회는 예산의 절반(기본 15초)과 남은 시작 시간 안에서만 기다립니다. 정리는 별도의 기존 제한을 유지합니다. 식별된 인증·인가·요청 한도·잘못된 인자·취소 오류와 유형을 알 수 없는 오류는 재시도하지 않습니다. 진단에는 제한된 오류 분류·코드만 남기고 상위 오류 메시지 원문은 남기지 않습니다. 읽기 전용 재시도는 프롬프트나 도구 결과를 다시 보내지 않습니다.

### 안전한 bridge 재시작

SDK 강제 종료가 실패하면 서버는 `upstream_cleanup_failed` 코드를 가진 `bridge.shutdown_failed`를 기록하고 종료 코드 1을 반환합니다. 전경 실행기도 비정상 종료하며, 상주 PID가 종료됐다는 사실만으로 SDK 정리가 확인되지는 않으므로 비공개 bridge 로그를 확인하세요. 정리가 성공했다고 간주하거나 이미 완료한 도구를 다시 실행하지 마세요.
