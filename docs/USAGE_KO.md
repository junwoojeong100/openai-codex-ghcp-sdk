...
| 목적 | 안내 |
| --- | --- |
| Codex 실행·재개 | [Codex 실행](#codex-실행) |
| 다른 저장소에서 작업 | [다른 프로젝트에서 사용](#다른-프로젝트에서-사용) |
| Codex 명령·옵션 전달 | [입력 위치](#codex-명령과-옵션-전달) |
| bridge 상주 실행·상태 확인·중지 | [선택적 상주 bridge](#선택적-상주-bridge) |
| `/model`과 문맥 한도 이해 | [모델 선택과 문맥](#모델-선택과-문맥) |
| 모델·포트·실행파일 경로 지정 | [실행기 설정](#실행기-설정) |
| Codex 없이 HTTP 서버 직접 관리 | [서버 직접 실행 (고급)](#서버-직접-실행-고급) |
| 오류 해결 | [시작과 설정](#시작과-설정) · [대화 중 오류](#대화-중-오류) · [404/409 코드](#대화-상태-오류-404-409) · [기능 제한](#기능-제한) |
...
기본 실행은 빈 루프백 포트에 bridge를 시작하고 Codex 종료 시 해당 bridge를 정리합니다. 작업이 끝난 Codex에서 `/quit`을 입력하면 정상 종료하며, 별도 중지 명령은 필요하지 않습니다.

### 다른 프로젝트에서 사용

bridge 저장소가 아니라 **Codex로 작업할 프로젝트에서 터미널을 여세요.** 실행기의 절대 경로를 사용하면 현재 작업 디렉터리를 유지합니다. clone 위치가 `$HOME/GitHub/openai-codex-ghcp-sdk`인 경우:

```bash
"$HOME/GitHub/openai-codex-ghcp-sdk/bin/codex-ghcp"
```

다른 위치에 clone했다면 경로를 바꾸세요. 절대 경로 뒤에도 동일한 옵션을 사용할 수 있으며 셸 연동은 필요하지 않습니다.
...
| 입력 | 입력할 위치 |
| --- | --- |
| 실행기 옵션: `--ghcp-model`, `--bridge-background` | 셸 명령의 `--` 앞 |
| Codex 명령·옵션: `exec`, `resume`, `--sandbox` | 셸 명령의 `--` 뒤 |
| 슬래시 명령: `/model`, `/compact`, `/quit` | 셸이 아니라 실행 중인 Codex 안 |

공식 CLI의 `--model`/`-m`은 거절하므로 `--ghcp-model`을 사용하세요. 실행기는 공급자·전송 설정 재정의도 거절하며 승인·샌드박스를 우회하지 않습니다.

Git 저장소 밖에서는 `exec` 뒤에 `--skip-git-repo-check`를 추가해 Git 검사만 생략할 수 있습니다.
...
대화 중 모델을 바꾸려면 **Codex 안에서** `/model`을 입력하세요. 계정에서 사용할 수 있는 모델만 정해진 순서로 표시합니다. 권한 변경은 다시 실행하여 반영하세요. 목록에 있다는 사실이 모든 기능의 지원을 뜻하지는 않으므로 [호환성](COMPATIBILITY_KO.md)을 확인하세요.

Codex 0.154.0의 모델 선택 화면에서 표시는 다음 뜻입니다.

| 표시 | 의미 |
| --- | --- |
| `(current)` | 현재 대화에서 사용하는 모델. |
| `(default)` | 목록의 첫 항목. 6개 모두 사용 가능하면 `claude-opus-5.5`이며, **실행기의 기본 모델이라는 뜻은 아닙니다.** |

**`/model`은 `~/.codex/config.toml`에 `model`과 `model_reasoning_effort`도 저장합니다.** 다음 실행에 미치는 영향은 실행 방법에 따라 다릅니다.

- **GHCP 실행기:** 여전히 `--ghcp-model`, `GHCP_MODEL`, `gpt-6-astra` 순으로 선택합니다.
- **공식 CLI 직접 실행:** `command codex`는 저장된 선택을 읽습니다. [선택적 zsh 연동](#선택적-zsh-연동)의 `codex-original`도 같으므로, 필요하면 저장된 모델을 먼저 되돌리세요.

임시 모델 목록은 상주 bridge를 사용하더라도 Codex 종료 시 삭제합니다. 저장된 선택까지 되돌리지는 않습니다. [목록 구현](ARCHITECTURE_KO.md#모듈)을 참고하세요.
...
- 실행기는 `-c` 인자로 Codex가 bridge에 연결하도록 설정하고, 생성한 **로컬 bridge 토큰**을 Codex 프로세스 환경 변수로 전달합니다. 이 토큰은 GitHub/OpenAI 인증정보가 아닙니다.
- bridge가 지원하지 않는 Codex 기능인 WebSocket·요청 압축·호스팅 웹 검색·원격 compaction·reasoning summary를 끕니다.
- bridge는 Copilot 런타임 자체의 MCP 서버(사용자·플러그인 서버 포함)를 끈 상태로 Copilot 세션을 시작합니다. Codex의 MCP 서버는 Codex가 직접 실행하므로 그대로 동작합니다. [도구 호출 왕복](ARCHITECTURE_KO.md#도구-호출-왕복)을 참고하세요.

실행기는 `~/.codex/config.toml`, `auth.json`, 셸 시작 파일이나 다른 프로젝트의 서버를 수정하지 않으며, Copilot/GitHub 로그인을 Codex로 복사하지 않습니다. 다른 Codex 설정은 그대로 적용되며 새 Codex 프로필을 만드는 것은 아닙니다. 예외는 사용자가 직접 선택하는 두 가지입니다. Codex의 `/model`은 선택을 `~/.codex/config.toml`에 저장할 수 있고, 선택적인 [zsh 연동](#선택적-zsh-연동)은 사용자가 `~/.zshrc`를 직접 수정하는 것입니다.
...
아래에서 해당 증상을 찾으세요. 전체 지원 범위는 [호환성](COMPATIBILITY_KO.md)에 있습니다.
...
| 제한 | 기본값 | 환경 변수 |
| --- | --- | --- |
| SDK 설정 | 30초 | `SDK_STARTUP_TIMEOUT_MS` |
| 첫 모델 진행 대기 | 180초 | `TURN_FIRST_PROGRESS_TIMEOUT_MS` |
| 진행 시작 후 무진행 상태 | 90초 | `TURN_IDLE_TIMEOUT_MS` |
| 복구를 포함한 전체 모델 턴 | 5분 | `TURN_TIMEOUT_MS` |
| 큐 대기를 포함한 전체 요청 | 6분 | `REQUEST_TIMEOUT_MS` |

제한 시간은 동시에 적용되며 서로 더해지지 않습니다. 값은 **밀리초** 단위이며, 바꾼 값은 [bridge를 재시작](#수정-코드-적용)해야 적용됩니다. keepalive는 모델 진행으로 세지 않습니다. 모든 설정과 오류 코드는 [운영 기본값](STABILITY_TESTING_KO.md#운영-기본값)을 참고하세요.

**자동 복구:** 모델 턴이 응답 없이 멈추면 bridge는 Codex의 응답 스트림을 유지한 채 Copilot 세션을 **요청당 한 번** 교체할 수 있습니다. 잃거나 반복될 작업이 없을 때만 복구합니다. 즉 SDK 연결이 정상이고, 입력 접수가 확인됐고, 모델 출력이나 도구 호출이 아직 없고, 이전 세션 정리가 확인돼야 합니다. 복구해도 원래 턴·요청 제한 시간은 그대로이고, 완료된 도구 결과를 다시 보내지 않으며, 추가 Copilot 사용량이 발생할 수 있습니다. 끄려면 `TURN_IDLE_RECOVERY_ATTEMPTS=0`을 사용합니다. Codex 자체의 HTTP·스트림 자동 재시도는 계속 꺼져 있습니다. [복구 상세](ARCHITECTURE_KO.md#모델-진행과-복구)를 참고하세요.
...
`ETIMEDOUT`·연결 실패라면 **제한 시간을 늘리기 전에 네트워크·프록시/VPN·서비스 상태를 확인하세요.** `/health.ready`는 로컬 SDK 연결 상태이며 원격 모델 서비스까지 보장하지 않습니다.

복구 후에도 `copilot_idle_timeout`이 나면 교체한 세션도 제한 시간까지 응답이 없었다는 뜻이며, 그것만으로 bridge가 멈췄다고 볼 수는 없습니다. 오류 메시지는 만료된 제한을 `phase: first_progress`(아직 진행 없음) 또는 `phase: streaming`(진행 후 멈춤)으로 표시합니다. 교체한 세션에 새 턴 예산을 주지는 않습니다.
...
기본값을 바꾸거나 끊김 방지를 보장하는 설정은 아니며 대기 시간·추론 사용량이 늘 수 있습니다. 무진행 제한만이 아니라 턴·요청 예산을 함께 조정하세요.

도구 결과까지 완료된 턴 사이에 `/compact`로 긴 이력을 줄이면 지연을 줄이는 데 도움이 됩니다. 최대 컨텍스트가 크다고 긴 대화가 빨라지는 것은 아닙니다. keepalive를 모델 진행으로 위장하거나 완료한 도구를 무조건 재실행하지 않습니다.

**장시간 검사를 실행 중이었다면 새 실행을 시작하기 전에 기존 `.runtime` 보고서를 확인하세요.** 검사 러너는 진행 기록을 독립적으로 저장합니다. 대화가 끊겼다고 검사 프로세스도 멈춘 것은 아닙니다.
