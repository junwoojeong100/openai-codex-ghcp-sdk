...
The connection incident also prompted content-free startup diagnostics identifying `start`, `ping` or `listModels`, and idle errors now include the configured interval and last root progress event. These changes improve diagnosis; they do not fabricate progress, replay uncertain tool results or change the default deadlines. The optional longer-wait launch is documented in the [main troubleshooting section](../../../README.md#boundaries-and-troubleshooting), but was **not** used in either scored run.
