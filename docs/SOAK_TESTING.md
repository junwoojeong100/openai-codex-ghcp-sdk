...
The standalone duration accepts 1–86,400 seconds and is a minimum measured traffic interval, not a five-hour certificate. The final in-flight turn has a bounded completion allowance. `Ctrl+C`/`SIGTERM` cancels work; it cannot turn a partial duration into a pass. Payloads accept 128–65,536 bytes; response requests accept 0–3,000 words (0 requests only the marker). Actual response size is recorded and checked against the declared minimum character workload, not assumed from the prompt. Models must be one of the bridge's seven supported IDs; there is no fallback.
